...
def main(spark):
    ...
    genome_scores = spark.read.csv('hdfs:/user/qy561_nyu_edu/ml-latest/genome-scores.csv', header=True, inferSchema=True)
    genome_tags = spark.read.csv('hdfs:/user/qy561_nyu_edu/ml-latest/genome-tags.csv', header=True, inferSchema=True)

    # Convert to Parquet and save to HDFS directory
    genome_scores.write.parquet('hdfs:/user/qy561_nyu_edu/ml-latest/genome-scores.parquet')
    genome_tags.write.parquet('hdfs:/user/qy561_nyu_edu/ml-latest/genome-tags.parquet')
